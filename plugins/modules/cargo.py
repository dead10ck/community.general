...
class Cargo:
    ...
    def _exec(self, args, run_in_check_mode=False, check_rc=True) -> Tuple[str, str]:  # noqa: UP006
    ...
    def get_installed(self) -> packages_type:
        if not os.path.exists(self._crates_json) or os.path.getsize(self._crates_json) == 0:
            return {}
    ...
    def get_latest(self, package: package_type, cache: bool = True) -> package_type:
        if cache and package["name"] in self._latest:
            return self._latest[package["name"]]
...
def main():
    ...
    if state == "present":
        ...
        git_installed_fields = {
            package_name: (
                {
                    key: val
                    for key, val in installed_packages.get(package_name, {}).get("git", {}).items()
    ...
    if argv:
        module.warn("changes are always assumed when using argv")
    ...
    if module._diff:
        ...
        for diff_obj in diff:
            for elt in diff_obj.values():
                if isinstance(elt, dict):
                    elt.pop("bin_stats", None)
